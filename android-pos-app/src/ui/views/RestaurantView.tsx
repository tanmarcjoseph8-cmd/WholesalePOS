...
import { Armchair, ArrowRightLeft, Check, Merge, Pencil, Plus, Power, Printer, RefreshCw, Scissors, Search, Trash2, Undo2, Users } from "lucide-react";
...
export function RestaurantView() {
  ...
  return (
    <section className="page-stack">
      ...
      <ConfirmDialog open={checkoutOpen} title={`Pay ${editing?.orderNumber ?? "order"}?`} confirmLabel={busy ? "Completing" : `Complete ${formatMoney(total)}`} disabled={busy || payment.dueCents > 0} onClose={() => setCheckoutOpen(false)} onConfirm={() => void checkout()}><div className="payment-grid"><label>Cash<input type="number" min="0" step="0.01" value={cash / 100} onChange={(event) => setCash(moneyInputToCents(event.target.value))} /></label><label>GCash<input type="number" min="0" step="0.01" value={gcash / 100} onChange={(event) => setGcash(moneyInputToCents(event.target.value))} /></label></div><div className="payment-balance"><div><span>Amount received</span><strong>{formatMoney(payment.paidCents)}</strong></div><div className={payment.changeCents > 0 ? "change" : "due"}><span>{payment.changeCents > 0 ? "Change" : "Amount due"}</span><strong>{formatMoney(payment.changeCents > 0 ? payment.changeCents : payment.dueCents)}</strong></div></div><p>Payment will consume reservations and deduct physical stock once.</p></ConfirmDialog>
