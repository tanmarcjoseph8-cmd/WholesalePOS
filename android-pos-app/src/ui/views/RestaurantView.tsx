...
import { Armchair, ArrowRightLeft, Check, Merge, Pencil, Plus, Power, RefreshCw, Scissors, Search, Trash2, Undo2, Users } from "lucide-react";
import { saleTotals, toBaseQuantity } from "../../domain/calculations";
...
export function RestaurantView() {
  ...
  async function checkout() {
    ...
    try {
      ...
      setEditing(null); setLines([]); setCheckoutOpen(false); refresh(); notify(`Table paid. Receipt ${sale.receiptNumber}.`, "success");
  ...
  return (
    <section className="page-stack">
      <header className="page-header"><div><h2>Restaurant</h2><p>Tables and unpaid orders use the same local products and inventory.</p></div><div className="header-actions order-create"><select aria-label="New order type" value={newOrderType} onChange={(event) => setNewOrderType(event.target.value as OrderType)}>{orderTypes.filter((type) => type !== "DINE_IN").map((type) => <option value={type} key={type}>{type.replaceAll("_", " ")}</option>)}</select><button className="button secondary" onClick={() => void createOrder(undefined, newOrderType)} disabled={busy}><Plus size={18} /> New order</button>{canManageTables ? <button className="button primary" onClick={() => void addTable()}><Armchair size={18} /> Table</button> : null}</div></header>
      ...
      <ConfirmDialog open={checkoutOpen} title={`Pay ${editing?.orderNumber ?? "order"}?`} confirmLabel={busy ? "Completing" : `Complete ${formatMoney(total)}`} disabled={busy || cash + gcash < total} onClose={() => setCheckoutOpen(false)} onConfirm={() => void checkout()}><div className="payment-grid"><label>Cash<input type="number" min="0" step="0.01" value={cash / 100} onChange={(event) => setCash(Math.round(Number(event.target.value) * 100))} /></label><label>GCash<input type="number" min="0" step="0.01" value={gcash / 100} onChange={(event) => setGcash(Math.round(Number(event.target.value) * 100))} /></label></div><p>Payment will consume reservations and deduct physical stock once.</p></ConfirmDialog>
    </section>
  );
}
