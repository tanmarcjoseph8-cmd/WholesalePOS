...
export function DashboardView() {
  ...
  const canViewStockAlerts = canViewInventory || user.permissions.includes("inventory.alerts.view");
  ...
  useEffect(() => {
    void Promise.all([app.settingsReports.dashboard(), canViewStockAlerts ? app.inventoryAlerts.listStockStatuses(user) : Promise.resolve([]), canUseDrawer ? app.cashDrawer.current(user) : Promise.resolve(null)]).then(([snapshot, statuses, drawer]) => {
      setData(snapshot);
  ...
  }, [app, user, revision, canViewStockAlerts, canUseDrawer]);
  ...
  return (
    <section className="page-stack">
      ...
      {canViewStockAlerts ? <section className="data-panel dashboard-stock-alerts">
        ...
        {stockAlerts.length ? <div>{stockAlerts.map((status) => <button className={!canViewInventory ? "readonly" : undefined} key={`${status.productId}-${status.warehouseId}`} disabled={!canViewInventory} onClick={() => { if (canViewInventory) openInventoryProduct(status.productId); }}><div><strong>{status.productName}</strong><span>{status.warehouseName}</span></div><div className="dashboard-stock-quantity"><b>{formatQuantity(status.currentQuantityMicro)} {status.inventoryUnit.toLowerCase()} left</b><span>Threshold {formatQuantity(status.thresholdMicro)}</span></div><span className={`status-label ${status.status.toLowerCase()}`}>{status.status.replaceAll("_", " ")}</span>{canViewInventory ? <ExternalLink size={18} /> : null}</button>)}</div> : <p className="empty-state">All products are above their low-stock thresholds.</p>}
      </section> : null}
