...
export function InventoryView() {
  ...
  return (
    <section className="page-stack">
      <header className="page-header"><div><h2>Inventory</h2><p>Products, physical stock, reservations, and permanent movement history.</p></div>{canManageInventory ? <div className="header-actions"><button className="button secondary" onClick={() => void addCategory()} disabled={busy}><Tag size={18} /> Category</button><button className="button secondary" onClick={() => void chooseImport()} disabled={busy}><FileSpreadsheet size={18} /> Import</button><button className="button primary" onClick={() => setEditor(emptyProduct())}><Plus size={18} /> Product</button></div> : null}</header>
