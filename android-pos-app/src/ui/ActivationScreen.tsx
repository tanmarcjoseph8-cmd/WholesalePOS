...
export function ActivationScreen({ app, status, onActivated }: { app: OfflinePosApplication; status: MobileLicenseStatus; onActivated: (status: MobileLicenseStatus) => void }) {
  ...
  async function startScanner() {
    ...
    try {
      ...
      controlsRef.current = await reader.decodeFromVideoDevice(undefined, videoRef.current, (result) => {
        ...
        } catch (caught) { setError(caught instanceof Error ? caught.message : "This QR code is not a Suki Sync activation."); }
      });
  ...
  const renewal = status.state === "EXPIRED";
  if (status.state === "CLOCK_INVALID") return <main className="activation-page"><section className="activation-panel"><header><div className="activation-mark"><ShieldCheck size={28} /></div><div><p className="eyebrow">Suki Sync</p><h1>Correct device time</h1></div></header><p className="renewal-message">System time appears incorrect. Please correct your device date and time. Your business data is safe.</p><div className="device-license-box"><div><small>Device ID</small><code>{status.deviceId}</code></div><button type="button" className="icon-button" title="Copy Device ID" aria-label="Copy Device ID" onClick={() => void navigator.clipboard.writeText(status.deviceId)}><Copy size={19} /></button></div><button className="button primary full" onClick={() => void app.license.getStatus("resume").then(onActivated)}>Check date and time again</button></section></main>;
  return <main className="activation-page"><section className="activation-panel"><header><div className="activation-mark"><ShieldCheck size={28} /></div><div><p className="eyebrow">Suki Sync</p><h1>{renewal ? "Renew this tablet" : "Activate this tablet"}</h1></div></header><p className={renewal ? "renewal-message" : "muted"}>{renewal ? "Your license has expired. Your business data is safe. Please obtain a renewal code to continue using the application." : "Activation is verified on this device without internet access."}</p>
    ...
    <div className="activation-meta"><span>App version <strong>{status.appVersion}</strong></span><span>Status <strong>{renewal ? "Expired" : "Activation required"}</strong></span></div>
    ...
    <form className="form-stack" onSubmit={submit}><label>{renewal ? "Renewal code" : "Activation code"}<textarea value={code} onChange={(event) => setCode(event.target.value)} rows={5} required autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="WPOS1..." /></label>{error ? <p className="form-error">{error}</p> : null}<button className="button primary" disabled={busy || !code.trim()}><KeyRound size={19} /> {busy ? "Verifying activation" : renewal ? "Apply renewal offline" : "Activate offline"}</button></form>
