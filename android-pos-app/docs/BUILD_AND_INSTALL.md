...
1. Copy `apk/WholesalePOS-Offline-0.4.1-release.apk` to the tablet.
...
```powershell
adb install -r .\apk\WholesalePOS-Offline-0.4.1-release.apk
```

Compare the APK with `apk/checksums.json`:

```powershell
Get-FileHash .\apk\WholesalePOS-Offline-0.4.1-release.apk -Algorithm SHA256
```
