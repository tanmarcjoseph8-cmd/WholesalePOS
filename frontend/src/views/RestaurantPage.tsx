...
type EditorItem = RestaurantOrderItemInput & { name: string; variant: string | null; inventoryUnit: string; taxRate: number };
...
function TableDialog({
  ...
  const [form, setForm] = useState({ number: table?.number ?? "", section: table?.section ?? "Main", capacity: table?.capacity ?? 2, notes: table?.notes ?? "" });
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-label={table ? "Edit table" : "Add table"}>
      ...
      >
        ...
        {save.error || disable.error ? <p className="mt-3 rounded-md bg-rose/10 p-3 text-sm font-semibold text-rose">{save.error?.message ?? disable.error?.message}</p> : null}
        <div className="mt-5 flex items-center justify-between gap-3">
          {table ? (
            <button className="focus-ring inline-flex h-10 items-center gap-2 rounded-md border border-rose/40 px-3 text-sm font-bold text-rose" type="button" disabled={disable.isPending || Boolean(table.activeOrder)} onClick={() => disable.mutate()}>
              <Trash2 size={16} /> Disable
            </button>
          ) : (
            <span />
          )}
          <button className="focus-ring h-10 rounded-md bg-ocean px-4 text-sm font-bold text-white" disabled={save.isPending}>
...
export function RestaurantPage({ permissions }: { permissions: string[] }) {
  ...
  const tables = useQuery({ queryKey: ["restaurant", "tables"], queryFn: () => fetchRestaurantTables(false) });
  ...
  const restaurantProducts = useMemo(() => (products.data?.items ?? []).filter((product) => product.salesChannel !== "RETAIL"), [products.data?.items]);
  ...
  const newOrder = useMutation({
    mutationFn: (input: { orderType: RestaurantOrderType; table?: RestaurantTable }) =>
      createRestaurantOrder({
  ...
  const saveOrder = useMutation({
    mutationFn: (status?: RestaurantOrderStatus) => {
      ...
      return updateRestaurantOrder({
        ...
        guestCount,
        note: orderNote || null,
        items: editorItems.map(({ name: _name, variant: _variant, inventoryUnit: _inventoryUnit, taxRate: _taxRate, ...item }) => item)
      });
    },
    onSuccess: async (order) => {
      await refreshRestaurant();
  ...
  const operationError = openOrder.error ?? newOrder.error ?? saveOrder.error ?? assignTables.error ?? cancelOrder.error ?? reopenOrder.error ?? checkout.error;
    ...
    <section className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select className="focus-ring h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-semibold dark:border-slate-700 dark:bg-slate-900" value={newOrderType} onChange={(event) => setNewOrderType(event.target.value as RestaurantOrderType)}>
            {Object.entries(orderTypeLabels).filter(([type]) => type !== "DINE_IN").map(([type, label]) => <option key={type} value={type}>{label}</option>)}
          </select>
          <button className="focus-ring inline-flex h-10 items-center gap-2 rounded-md bg-ocean px-3 text-sm font-bold text-white" onClick={() => newOrder.mutate({ orderType: newOrderType })}>
      ...
      {view === "tables" ? (
        <div className="space-y-5">
          ...
          {[...new Set((tables.data ?? []).map((table) => table.section))].map((section) => (
            <section key={section}>
              <h3 className="mb-3 text-sm font-bold uppercase text-slate-500">{section}</h3>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                {tables.data?.filter((table) => table.section === section).map((table) => (
          ...
          {!tables.isLoading && !tables.data?.length ? <div className="rounded-md border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500 dark:border-slate-700">No tables yet. Add the first table to begin dine-in service.</div> : null}
        </div>
      ) : (
            ...
            <div className="mt-4 max-h-[65vh] space-y-2 overflow-auto">
              {orders.data?.items.map((order) => (
                <button key={order.id} className={`focus-ring w-full rounded-md border p-3 text-left ${editingOrder?.id === order.id ? "border-ocean bg-ocean/5" : "border-slate-200 dark:border-slate-700"}`} onClick={() => closedOrderStatuses.includes(order.status) ? loadEditor(order) : openOrder.mutate(order)}>
                  ...
                  <p className="mt-1 text-xs text-slate-500">{orderTypeLabels[order.orderType]}{order.primaryTable ? ` · Table ${order.primaryTable.number}` : ""}</p>
            ...
            <div className="space-y-4">
              <section className="rounded-md border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div><p className="text-xs font-bold uppercase text-ocean">{orderTypeLabels[editingOrder.orderType]}</p><h3 className="mt-1 text-xl font-bold">{editingOrder.orderNumber}</h3><p className="mt-1 flex items-center gap-1 text-xs text-slate-500"><Clock3 size={13} /> Updated {new Date(editingOrder.updatedAt).toLocaleTimeString()}</p></div>
              ...
              <section className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  <div className="mt-4 space-y-3">
                    {editorItems.map((item) => (
                      <div key={item.productId} className="grid gap-3 border-b border-slate-100 pb-3 dark:border-slate-800 sm:grid-cols-[1fr_150px_44px]">
                        ...
                        <button className="focus-ring grid h-9 w-9 place-items-center rounded-md text-rose" onClick={() => setEditorItems((current) => current.filter((entry) => entry.productId !== item.productId))} aria-label={`Remove ${item.name}`}><Trash2 size={17} /></button>
                      </div>
                    ))}
                ...
                <aside className="rounded-md border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <div className="mt-3 max-h-80 space-y-2 overflow-auto">{restaurantProducts.map((product) => <button key={product.id} className="focus-ring w-full rounded-md border border-slate-200 p-3 text-left dark:border-slate-700" onClick={() => addProduct(product)} disabled={productStock(product) <= 0}><div className="flex justify-between gap-2"><span className="font-semibold">{product.name}</span><span className="text-sm">{formatCurrency(product.retailPrice)}</span></div><p className="mt-1 text-xs text-slate-500">{productStock(product).toLocaleString()} {product.inventoryUnit.toLowerCase()} available</p></button>)}</div>
                </aside>
              </section>
              ...
              <section className="grid gap-4 2xl:grid-cols-2">
                <div className="rounded-md border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">{tables.data?.map((table) => {
                  ...
                  <button className="focus-ring mt-4 h-9 rounded-md border border-ocean px-3 text-sm font-bold text-ocean" onClick={() => assignTables.mutate()} disabled={!primaryTableId || !selectedTableIds.length}>Move / join tables</button>
                </div>
                <div className="rounded-md border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
